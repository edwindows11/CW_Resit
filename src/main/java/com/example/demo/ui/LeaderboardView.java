...
public class LeaderboardView extends VBox {
    ...
    public LeaderboardView(Runnable onBack) {
        ...
        getChildren().addAll(buildTitle(), buildTable(), buildBackButton(onBack));
    }
    ...
    private Text buildTitle() {
        ...
        t.setEffect(new DropShadow(20, Color.web("#ffd700")));
        return t;
    }
    ...
    private VBox buildTable() {
        ...
        if (entries.isEmpty()) {
            ...
            empty.setFont(Font.font("Segoe UI", 18)); empty.setFill(Color.web("#a0a0b0"));
            table.getChildren().add(empty); table.setAlignment(Pos.CENTER); return table;
        }
        table.getChildren().add(buildRow("#", "Name", "Score", "Mode", "Date", true));
        int rank = 1;
        for (LeaderboardEntry e : entries)
            table.getChildren().add(buildRow(String.valueOf(rank++), e.playerName(),
                    String.valueOf(e.score()), e.mode().getDisplayName(), e.formattedTimestamp(), false));
        return table;
    }

    private HBox buildRow(String rank, String name, String score, String mode, String date, boolean isHeader) {
        Color textColor = isHeader ? Color.web("#a0a0b0") : Color.web("#eaeaea");
        FontWeight wt   = isHeader ? FontWeight.BOLD : FontWeight.NORMAL;
        HBox row = new HBox(12, cell(rank, 50, wt, textColor), cell(name, 160, wt, textColor),
                cell(score, 120, wt, isHeader ? textColor : Color.web("#ffd700")),
                cell(mode, 180, wt, textColor), cell(date, 140, wt, textColor));
        row.setAlignment(Pos.CENTER_LEFT);
        row.setPadding(new Insets(8, 16, 8, 16));
        row.setStyle("-fx-background-color: " + (isHeader ? "#16213e" : "transparent") + "; -fx-background-radius: 6;");
        return row;
    }
    ...
    private Label cell(String text, double width, FontWeight weight, Color color) {
        ...
        lbl.setTextFill(color); lbl.setPrefWidth(width); lbl.setMinWidth(width); return lbl;
    }

    private Button buildBackButton(Runnable onBack) {
        String base = btnStyle("#0f3460"); String hover = btnStyle("#e94560");
        Button btn = new Button("← Back to Menu"); btn.setStyle(base);
        btn.setPrefWidth(220); btn.setPrefHeight(44);
        btn.setOnMouseEntered(e -> btn.setStyle(hover));
        btn.setOnMouseExited(e  -> btn.setStyle(base));
        btn.setOnAction(e -> onBack.run()); return btn;
    }
