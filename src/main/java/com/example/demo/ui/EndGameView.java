...
/**
 * Screen displayed after a game session ends, showing the outcome (win or loss),
 * the final score, a name-entry field for the leaderboard, and navigation buttons.
 *
 * <p>When the player confirms their name the entry is added to the in-memory
 * {@link Leaderboard} via the provided callback.</p>
 */
public class EndGameView extends VBox {
    ...
    /**
     * Builds the end-game screen.
     *
     * @param score          the player's final score
     * @param mode           the mode that was played
     * @param won            {@code true} if the player reached the win tile
     * @param onSaveAndMenu  callback invoked with {@code (playerName, score)}
     *                       when the player saves and returns to the menu
     * @param onPlayAgain    callback invoked when the player restarts the same mode
     */
    public EndGameView(long score, GameMode mode, boolean won,
                       BiConsumer<String, Long> onSaveAndMenu,
        ...
        Text headline = buildHeadline(won);
        Text scoreText = buildScoreText(score);
        Text modeText  = buildModeText(mode);

        // Name entry
        Text namePrompt = smallText("Save your score to the leaderboard:");
    ...
    private Text buildHeadline(boolean won) {
        ...
        DropShadow glow = new DropShadow(25, from);
        glow.setInput(new Glow(0.5));
        t.setEffect(glow);
        return t;
    }
    ...
    private Text buildScoreText(long score) {
        ...
        t.setFill(Color.web("#eaeaea"));
        return t;
    }

    private Text buildModeText(GameMode mode) {
        Text t = new Text(mode.getDisplayName());
        t.setFont(Font.font("Segoe UI", FontWeight.NORMAL, 16));
        t.setFill(Color.web("#a0a0b0"));
        return t;
    }
